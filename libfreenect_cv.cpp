...
IplImage *freenect_sync_get_depth_cv(int index)
{
	...
	if (!image)
		image = cvCreateImage(cvSize(640, 480), IPL_DEPTH_16U, 1);
	unsigned int timestamp;
	if (freenect_sync_get_depth
		((void **)&data, &timestamp, index, FREENECT_DEPTH_REGISTERED))
		return NULL;
	...
	for (i = 0; i < 640; i++)
		for (j = 0; j < 480; j++)
			targetData[i * 480 + j] =
				0xFFFF - ((unsigned short int *)data)[i * 480 + j] * 32;
	// NOTE: Multiplying 16 may occur overflow in large distance but it might
...
IplImage *freenect_sync_get_rgb_cv(int index)
{
	...
	if (freenect_sync_get_video
		((void **)&data, &timestamp, index, FREENECT_VIDEO_RGB))
		return NULL;
...
IplImage *freenect_sync_get_ir_cv(int index)
{
	...
	unsigned int timestamp;
	if (freenect_sync_get_video
		((void **)&data, &timestamp, index, FREENECT_VIDEO_IR_8BIT))
		return NULL;
	cvSetData(image, data, 640);
	return image;
}
